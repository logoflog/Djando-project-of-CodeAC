...
#从csv文件中读取数据
csv2db = pd.read_csv("数据源.csv")
# 创建对象
mySqlite = SqliteTool()
# 插入数据
for index, row in csv2db.iterrows():
    # 一次插入一条数据
    mySqlite.operate_one('insert into datasource values(?,?,?,?,?,?,?,?,?)',
                         (str(row['STATION'])+','+row['DATE'],
                          ...
                          row['WND']))
# 一次插入多条数据
# mySqlite.operate_many('insert into info values(?,?,?)',[(5, 'Alice', 22),(6, 'John', 21)])
# 关闭游标和连接
mySqlite.close_con()